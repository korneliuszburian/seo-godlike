...
test("agency report freshness ignores non-current historical GSC periods", async () => {
  const root = await mkdtemp(join(tmpdir(), "seo-godlike-agency-freshness-history-test-"));
  try {
    const artifacts = join(root, "artifacts");
    await mkdir(artifacts);
    await writeAgencySelectionBundle(artifacts, "gsc-history", "2026-07-28T08:00:00.000Z", 2, { start: "2026-08-01", end: "2026-08-28" });
    await writeAgencySelectionBundle(artifacts, "gsc-current", "2026-07-29T08:00:00.000Z", 3, { start: "2026-07-01", end: "2026-07-28" });
    ...
    const request: AhrefsAnalyticsRequest = { schema_version: "1", run_id: "ahrefs-current", client_id: "bodymove", property_id: "bodymove.pl", provider: "ahrefs", operation: "site-explorer.metrics", metric: "org_traffic", date_range: { start: "2026-07-28", end: "2026-07-28" }, credential_ref: "keyring:seo-godlike/ahrefs-api-key", policy_mode: "read_only", captured_at: "2026-07-29T08:00:00.000Z" };
    await runAhrefsAnalytics(request, registry, capabilities, JSON.stringify({ metrics: { org_traffic: 10, org_keywords: 2, org_keywords_1_3: 1 } }), join(artifacts, "ahrefs-current"));
...
test("agency report fails closed when an Ahrefs snapshot has no selected GSC baseline", async () => {
  const root = await mkdtemp(join(tmpdir(), "seo-godlike-agency-missing-freshness-baseline-test-"));
  try {
    const artifacts = join(root, "artifacts");
    await mkdir(artifacts);
    const registry: ClientRegistry = { clients: [{ client_id: "bodymove", display_name: "Bodymove", properties: [{ property_id: "bodymove.pl", provider: "ahrefs", canonical_property: true }] }] };
    const capabilities: CapabilityRegistry = { capabilities: [{ capability_id: "ahrefs.site-explorer.metrics", provider: "ahrefs", operation_id: "site-explorer.metrics", api_version: "v3", read_write: "read", state: "schema_verified" }] };
    const request: AhrefsAnalyticsRequest = { schema_version: "1", run_id: "ahrefs-without-gsc", client_id: "bodymove", property_id: "bodymove.pl", provider: "ahrefs", operation: "site-explorer.metrics", metric: "org_traffic", date_range: { start: "2026-07-28", end: "2026-07-28" }, credential_ref: "keyring:seo-godlike/ahrefs-api-key", policy_mode: "read_only", captured_at: "2026-07-29T08:00:00.000Z" };
    await runAhrefsAnalytics(request, registry, capabilities, JSON.stringify({ metrics: { org_traffic: 10, org_keywords: 2, org_keywords_1_3: 1 } }), join(artifacts, "ahrefs"));
    const scope: ScopePlan = { schema_version: "1", generated_at: "2026-07-30T00:00:00.000Z", status: "ready", entries: [
      { client_id: "bodymove", client_display_name: "Bodymove", property_id: "sc-domain:bodymove.pl", provider: "google-search-console", status: "ready", reason: null, metrics: [] },
      { client_id: "bodymove", client_display_name: "Bodymove", property_id: "bodymove.pl", provider: "ahrefs", status: "ready", reason: null, metrics: [] },
    ] };
    ...
    const ahrefsStatus = summary.source_status.find((source) => source.provider === "ahrefs");
    assert.equal(ahrefsStatus?.status, "unavailable");
    assert.equal(ahrefsStatus?.reason_code, "missing_freshness_baseline");
    assert.equal(summary.report_status, "blocked");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
...
test("agency report does not use another host's GSC period as an Ahrefs freshness baseline", async () => {
  const root = await mkdtemp(join(tmpdir(), "seo-godlike-agency-cross-host-freshness-test-"));
  try {
    const artifacts = join(root, "artifacts");
    await mkdir(artifacts);
    await writeAgencySelectionBundle(artifacts, "gsc-krakow", "2026-07-29T08:00:00.000Z", 3, { start: "2026-07-01", end: "2026-07-28" }, "bodymove", "https://krakow.bodymove.pl/");
    const registry: ClientRegistry = { clients: [{ client_id: "bodymove", display_name: "Bodymove", properties: [{ property_id: "www.bodymove.pl", provider: "ahrefs", canonical_property: true }] }] };
    const capabilities: CapabilityRegistry = { capabilities: [{ capability_id: "ahrefs.site-explorer.metrics", provider: "ahrefs", operation_id: "site-explorer.metrics", api_version: "v3", read_write: "read", state: "schema_verified" }] };
    const request: AhrefsAnalyticsRequest = { schema_version: "1", run_id: "ahrefs-cross-host", client_id: "bodymove", property_id: "www.bodymove.pl", provider: "ahrefs", operation: "site-explorer.metrics", metric: "org_traffic", date_range: { start: "2026-07-28", end: "2026-07-28" }, credential_ref: "keyring:seo-godlike/ahrefs-api-key", policy_mode: "read_only", captured_at: "2026-07-29T08:00:00.000Z" };
    await runAhrefsAnalytics(request, registry, capabilities, JSON.stringify({ metrics: { org_traffic: 10, org_keywords: 2, org_keywords_1_3: 1 } }), join(artifacts, "ahrefs"));
    const scope: ScopePlan = { schema_version: "1", generated_at: "2026-07-30T00:00:00.000Z", status: "ready", entries: [
      { client_id: "bodymove", client_display_name: "Bodymove", property_id: "sc-domain:bodymove.pl", provider: "google-search-console", status: "ready", reason: null, metrics: [] },
      { client_id: "bodymove", client_display_name: "Bodymove", property_id: "https://krakow.bodymove.pl/", provider: "google-search-console", status: "ready", reason: null, metrics: [] },
      { client_id: "bodymove", client_display_name: "Bodymove", property_id: "www.bodymove.pl", provider: "ahrefs", status: "ready", reason: null, metrics: [] },
    ] };
    const summary = await writeAgencyReport(artifacts, join(root, "report"), scope, "2026-07-30T00:00:00.000Z", { sources: [] });
    const ahrefsStatus = summary.source_status.find((source) => source.provider === "ahrefs");
    assert.equal(ahrefsStatus?.status, "unavailable");
