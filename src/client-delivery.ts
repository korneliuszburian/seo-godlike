...
function unitHtml(unit: DeliveryUnit, generatedAt: string): string {
  ...
  const actionRows = unit.content?.actions.map((action) => `<tr><td>${escapeHtml(action.period.start)} — ${escapeHtml(action.period.end)}</td><td>${escapeHtml(action.type)}</td><td>${escapeHtml(action.status)}</td><td>${escapeHtml(action.title)}</td><td>${escapeHtml(action.target_url ?? "—")}</td></tr>`).join("") ?? "";
  const glossaryRows = unit.content?.glossary.map((entry) => `<tr><td>${escapeHtml(entry.term)}</td><td>${escapeHtml(entry.explanation)}</td></tr>`).join("") ?? "";
  ...
  const contentSection = unit.content ? `<section class="section"><div class="eyebrow">DZIAŁANIA DLA STRONY</div><h2>Wykonane i zaplanowane działania</h2><p class="muted">Rejestr działań pochodzi z jawnego inputu operatora; pozycje nie są wywnioskowane z metryk.</p><div class="table-wrap"><table><thead><tr><th>Okres</th><th>Typ</th><th>Status</th><th>Opis</th><th>Adres</th></tr></thead><tbody>${actionRows || `<tr><td colspan="5" class="no-data">Brak wpisów dla tego okresu.</td></tr>`}</tbody></table></div>${unit.content.contact ? `<h3>Kontakt</h3><p>${escapeHtml(unit.content.contact.name)}${unit.content.contact.email ? ` · ${escapeHtml(unit.content.contact.email)}` : ""}${unit.content.contact.phone ? ` · ${escapeHtml(unit.content.contact.phone)}` : ""}</p>` : ""}</section><section class="section"><div class="eyebrow">PRZYDATNE POJĘCIA</div><h2>Słownik raportu</h2><div class="table-wrap"><table><thead><tr><th>Pojęcie</th><th>Wyjaśnienie</th></tr></thead><tbody>${glossaryRows || `<tr><td colspan="2" class="no-data">Brak wpisów słownika.</td></tr>`}</tbody></table></div></section>` : "";
