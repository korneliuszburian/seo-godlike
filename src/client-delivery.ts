...
function unitHtml(unit: DeliveryUnit, generatedAt: string): string {
  ...
  const previousLabel = comparisonMetric?.previous_range
    ? `Porównanie: ${comparisonMetric.previous_range.start} — ${comparisonMetric.previous_range.end} względem bieżącego okresu. Zmiany pokazujemy wyłącznie dla sąsiednich, kompletnych zakresów.`
    : "Brak porównywalnej bazy — poprzedni okres nie jest kompletnym, bezpośrednio poprzedzającym zakresem.";
