...
function unitHtml(unit: DeliveryUnit, generatedAt: string): string {
  ...
  const cards = gsc.flatMap((metric) => [
    [`Observed — Google Search Console · ${metric.property_id} · Kliknięcia`, formatNumber(metricValue(metric, "clicks")), "Wartość z bieżącego zakresu źródłowego"],
    [`Observed — Google Search Console · ${metric.property_id} · Wyświetlenia`, formatNumber(metricValue(metric, "impressions")), "Wartość z bieżącego zakresu źródłowego"],
    [`Observed — Google Search Console · ${metric.property_id} · CTR`, formatPercent(metricValue(metric, "ctr")), "Wartość z bieżącego zakresu źródłowego"],
    [`Observed — Google Search Console · ${metric.property_id} · Średnia pozycja`, metricValue(metric, "position")?.toFixed(2) ?? "—", "Wartość z bieżącego zakresu źródłowego; niższa pozycja jest lepsza"],
  ...
  const previousLabel = "Historia pozostaje zachowana w pakiecie operatorskim; ten dokument pokazuje bieżące wyniki bez sztucznego porównania.";
