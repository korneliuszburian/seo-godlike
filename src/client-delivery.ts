...
function renderClientSwitcher(current: DeliveryUnit, links: UnitNavigationLink[]): string {
  const items = links.map((link) => `<a class="client-dot${link.id === current.id ? " active" : ""}" href="${escapeHtml(link.href)}" aria-label="${escapeHtml(link.title)}"${link.id === current.id ? ` aria-current="page"` : ""}><span>${escapeHtml(unitInitials(link.title, link.id))}</span><small>${escapeHtml(link.title)}</small></a>`).join("");
  return `<nav class="client-switcher" aria-label="Przełącz raport"><span class="switcher-label">Klient / domena</span>${items}</nav>`;
}
...
function unitHtml(unit: DeliveryUnit, generatedAt: string, clientLinks: UnitNavigationLink[]): string {
...
function appendClientContent(html: string, content: ClientContent | null, rankMonitoring: RankMonitoringSnapshot | null, rankHistory: RankHistoryComparison[], history: ProviderHistoryEntry[], includeClientSections: boolean, sources: DeliveryUnit["sources"]): string {
  ...
  const section = `${historySection(history)}${rankHistorySection}${rankSection}${actionsSection}`
    .replace('<section class="section"><div class="eyebrow">HISTORIA WYNIKÓW', '<section id="history" class="section supplement" style="margin-left:18mm;margin-right:18mm"><div class="eyebrow">HISTORIA WYNIKÓW')
    .replace('<section class="section"><div class="eyebrow">HISTORIA MONITORINGU', '<section id="rank-history" class="section supplement" style="margin-left:18mm;margin-right:18mm"><div class="eyebrow">HISTORIA MONITORINGU')
    .replace('<section class="section"><div class="eyebrow">MONITORING FRAZ', '<section id="ranking" class="section supplement" style="margin-left:18mm;margin-right:18mm"><div class="eyebrow">MONITORING FRAZ')
    .replace('<section class="section"><div class="eyebrow">DZIAŁANIA DLA STRONY', '<section id="actions" class="section supplement" style="margin-left:18mm;margin-right:18mm"><div class="eyebrow">DZIAŁANIA DLA STRONY');
...
export async function writeClientDelivery(options: ClientDeliveryOptions): Promise<ClientDeliveryResult> {
  ...
  const unitSegments = new Map(units.map((unit) => [unit.id, safeSegment(unit.id)]));
  for (const unit of units) {
    ...
    const navigationLinks = units.map((linkUnit) => ({
      id: linkUnit.id,
      title: linkUnit.title,
      kind: linkUnit.kind,
      href: relative(unitDir, join(outputDir, unitSegments.get(linkUnit.id) ?? safeSegment(linkUnit.id), `${safeSegment(linkUnit.id)}-seo-report.html`)),
    }));
    const html = appendClientContent(omitEmptyEvidenceSections(unitHtml(unit, generatedAt, navigationLinks), unit), unit.content, unit.rankMonitoring, unit.rankHistory, unit.history, unit.kind === "client", unit.sources);
  ...
  </style></head><body><main class="shell"><div class="topline"><span class="brand">Rekurencja.com · SEO intelligence</span><span class="badge">lokalny pakiet · bez ponownych zapytań</span></div><section class="hero"><div class="eyebrow">PANEL RAPORTOWY</div><h1>Wyniki SEO.<br>Bez szumu.</h1><p>Minimalistyczny dostęp do raportów klientów i domen. Każdy widok powstał z istniejących, zweryfikowanych artefaktów; nie wykonano ponownych zapytań do dostawców.</p></section><section class="dashboard"><div class="panel"><div class="eyebrow">KLIENCI I DOMENY</div><h2>Wybierz widok</h2><p>Przełączaj raporty jednym kliknięciem.</p><div class="units">${resultUnits.map((unit) => `<a class="unit" href="${escapeHtml(unit.html)}"><span class="dot">${escapeHtml(unitInitials(unit.id, unit.id))}</span><span><strong>${escapeHtml(unit.id)}</strong><small>${escapeHtml(unit.kind === "client" ? "raport klienta" : "domena oczekująca na przypisanie")}</small></span></a>`).join("")}</div></div><div class="panel"><div class="eyebrow">DOSTĘP</div><h2>Artefakty</h2><p>Wybierz format odpowiedni do pracy operatora lub prezentacji.</p><div class="actions"><a href="${escapeHtml(resultUnits[0]?.html ?? "#")}">Otwórz HTML</a>${resultUnits[0]?.pdf ? ` · <a href="${escapeHtml(resultUnits[0].pdf)}">Pobierz PDF</a>` : ""}</div><p>${resultUnits.map((unit) => `<a href="${escapeHtml(unit.email)}">Draft email: ${escapeHtml(unit.id)}</a>`).join(" · ")}</p><p class="footer">Pełne dane, hashe i ograniczenia znajdują się w manifeście tego pakietu.</p></div></section></main></body></html>`;
