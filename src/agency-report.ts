...
function markdown(summary: AgencyReportSummary, details: string[]): string {
  return [
    ...
    "| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: |",
    ...summary.cross_source_context.slice(0, 100).map((entry) => `| ${entry.client_id} | ${entry.key_type} | ${entry.join_type} | ${entry.key} | ${entry.gsc?.clicks ?? "—"} | ${entry.gsc?.impressions ?? "—"} | ${entry.gsc ? entry.gsc.position.toFixed(2) : "—"} | ${entry.ahrefs?.estimated_traffic ?? "—"} | ${entry.ahrefs?.position ?? "—"} |`),
    "",
...
function html(summary: AgencyReportSummary, details: string[]): string {
  ...
  const contextRows = summary.cross_source_context.slice(0, 100).map((entry) => `<tr>${[entry.client_id, entry.key_type, entry.join_type, entry.key, entry.gsc?.clicks ?? "—", entry.gsc?.impressions ?? "—", entry.gsc ? entry.gsc.position.toFixed(2) : "—", entry.ahrefs?.estimated_traffic ?? "—", entry.ahrefs?.position ?? "—"].map((value) => `<td>${escapeHtml(String(value))}</td>`).join("")}</tr>`).join("\n");
  const contextSection = `<h2>Cross-source context</h2><p>GSC values are observed Search Console metrics; Ahrefs values are estimated ranking/traffic context. They are not added together. Join type distinguishes matched, GSC-only, and Ahrefs-only rows.</p><table><thead><tr><th>Client</th><th>Type</th><th>Join</th><th>Key</th><th>GSC clicks</th><th>GSC impressions</th><th>GSC position</th><th>Ahrefs traffic</th><th>Ahrefs position</th></tr></thead><tbody>${contextRows}</tbody></table>`;
